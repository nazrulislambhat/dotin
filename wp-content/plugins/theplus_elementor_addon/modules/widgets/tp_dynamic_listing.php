...
class ThePlus_Dynamic_Listing extends Widget_Base {
	...
	protected function render() {
		...
		if ( ! $query->have_posts() && $settings["blogs_post_listing"] != 'acf_repeater' ) {			
			...
			$postattr=[];
			$data_loadkey=$serchAttr=$TotProduct='';			
			if( (($MorePostOptions == 'load_more' || $MorePostOptions == 'lazy_load' ) && $layout != 'carousel') || ( $FilterType == 'search_list' ) ){
			...
			$output .= '<div id="'.esc_attr($uid).'" class="tp-row post-inner-loop '.esc_attr($uid).' '.esc_attr($content_alignment).' '.esc_attr($content_alignment_3).' '.$preloader_cls.' tp_list" '.$serchAttr.' '.$TotProduct.'">';
			...
			if($settings["blogs_post_listing"] != 'acf_repeater' ){
				...
				if( have_rows($settings['acf_repeater_field_list'], $post_data->ID) ){
					while( have_rows($settings['acf_repeater_field_list'], $post_data->ID) ) {
						the_row();
							...
							if(!empty($child_categories)){
								$category_filter .= '<ul class="category-filters-child cate-parent-'.$par['slug'].'">';
									foreach($child_categories as $child):
										...
										$category_filter .= '<li><a href="' . get_term_link( $child, $post_taxonomies ) . '" class="filter-category-list"  data-filter=".'.esc_attr($term->slug).'">' .$featured_image_child. $term->name . '</a></li>';
									endforeach;
								$category_filter .= '</ul>';
							}
						endforeach;
